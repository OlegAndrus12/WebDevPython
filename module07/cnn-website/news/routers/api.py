...
async def list_articles(
    ...
    `q` searches what we have stored, which is not what `/?q=` does -- that one
    asks NewsAPI. A story NewsAPI has never handed us is not here.
    """
    query = q.strip()
    ...
    return ArticlePage(
        # ORM objects; ArticleOut has from_attributes=True. `source` is already
        # loaded by the repository's joinedload, so nothing lazy-loads here.
