...
# AsyncClient, so a NewsAPI call yields the event loop instead of blocking it.
# One per process, closed from the app's lifespan.
...
async def _get(endpoint: str, params: dict) -> list[dict]:
    ...
    # NewsAPI also returns 200 with an error body, so the status code alone is
